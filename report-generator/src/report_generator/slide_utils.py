...
# Static logic to manipulate PowerPoint slides
class SlideUtils:
    ...
    def update_paragraph(paragraph, placeholder_id, replacement_text, font={}):
        if paragraph:
            ...
            SlideUtils.merge_runs_with_same_formatting(paragraph)
            ...
            if run_with_placeholder:
                ...
                SlideUtils.update_run_font(run_with_placeholder, font)
            else:
                logging.debug(f"Could not find placeholder {placeholder_id} in the template. Skipping. (This may not be harmful. Tour template may not need that placeholder value)")
    
    def find_shapes_with_text(presentation, search_text):
        shapes = []
        for slide in presentation.slides:
            paragraphs = SlideUtils.find_text_in_slide(slide, search_text)
    ...
    def find_text_in_slide(slide, search_text):
        paragraphs = []
        for shape in slide.shapes:
            result = SlideUtils.find_text_in_shape(shape, search_text)
            if result:
    ...
    def find_text_in_shape(shape, search_text):
        ...
        if shape.shape_type == MSO_SHAPE_TYPE.GROUP:
            for s in shape.shapes:
                result = SlideUtils.find_text_in_shape(s, search_text)
                if result:
                    return result
    ...
    def add_content_paragraph(self, text_frame, markers, content, paragraph = None):
        ...
        for marker in markers:
            SlideUtils.set_sig_marker(paragraph, marker)
    ...
    def merge_runs_with_same_formatting(paragraph):
        ...
        for run in paragraph.runs:
            if last_run is None:
                last_run = run
                continue
            if SlideUtils.has_same_formatting(run, last_run):
                last_run = SlideUtils.combine_runs(last_run, run)
                continue
            last_run = run
    ...
    def set_cell_border(cell, border_color, border_width, border_style, *border_types):
        ...
        def fill_line(line, border_color, border_style):
            line_fill = SlideUtils.add_xml_element(line, 'a:solidFill')
            line_rgb = SlideUtils.add_xml_element(line_fill, 'a:srgbClr', val=border_color)
            line_style = SlideUtils.add_xml_element(line, 'a:prstDash', val=border_style)
            line_round_ = SlideUtils.add_xml_element(line, 'a:round')
            line_headEnd = SlideUtils.add_xml_element(line, 'a:headEnd', type='none', w='med', len='med')
            line_tailEnd = SlideUtils.add_xml_element(line, 'a:tailEnd', type='none', w='med', len='med') 
        ...
        for border_type in border_types:
            ...
            line = SlideUtils.add_xml_element(cell_xml_parent, tag, w=str(border_width), cap='flat', cmpd='sng', algn='ctr')
