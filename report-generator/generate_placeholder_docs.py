...
import pandas as pd
from report_generator.generator.placeholders import Placeholder, PlaceholderCollection, placeholders as all_placeholders
...
def get_placeholder_row_data(placeholder: Placeholder, skip_columns: Set[str] = None) -> dict:
    ...
    all_data = {
        ...
        'Description': inspect.getdoc(placeholder),
...
def generate_documentation():
    ...
    doc.add(Paragraph("Use these placeholders anywhere in your PowerPoint/Word template. `report-generator` will replace them with their actual value"))
    text_placeholders = [placeholder for placeholder in all_placeholders if placeholder.__doc_metadata__.type == PlaceholderDocType.TEXT]
    ...
    chart_placeholders = [placeholder for placeholder in all_placeholders if placeholder.__doc_metadata__.type == PlaceholderDocType.CHART]
    doc.add(Table(placeholders_to_table(chart_placeholders)))

    doc.add(Header("Other Placeholders"))
    other_placeholders = [placeholder for placeholder in all_placeholders if placeholder.__doc_metadata__.type == PlaceholderDocType.OTHER]
